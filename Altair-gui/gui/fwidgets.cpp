                     ...
                     const QStringList &stringList, const Q2ListWidget *enabledObjects,  const Q2ListWidget *disabledObjects)
{
    ...
    rank=0;
    int boxLabelSize=boxLabelList.size();
    if (boxLabelSize < (size+1))
        optionLabelStringList=optionLabelStringList.mid(0, boxLabelSize-1);
    else
        if (boxLabelSize > (size+1))
            for (int i=0; i < boxLabelSize-(size+1); i++)
            optionLabelStringList=optionLabelStringList << "";
...
FColorButton::FColorButton(const char* text, const QString  &color, QCheckBox* p)
{
    widgetDepth="0";
    ...
    parent = p;
    if(parent)  connect(parent, SIGNAL(toggled(bool)), colorLabel, SLOT(setVisible(bool)));
...
{
    widgetDepth="1";
    button[0]=new FColorButton(textR, DEFAULT_COLOR_0, parent); // red RGB
    button[1]=new FColorButton(textG, DEFAULT_COLOR_1, parent); // green RGB
    button[2]=new FColorButton( textB, DEFAULT_COLOR_2, parent); //blue RGB
