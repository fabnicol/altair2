...
test <- function(prime, Paie_I, Paie_B = NULL, Lignes_B = NULL, expr.rég ="", verbeux = FALSE) {
...
if (prime$catégorie != ""){
  ...
  if (expr.rég != "") {
    ...
    A.non.cat <- Lignes_A[! Catégorie %chin% prime$catégorie | ! grepl(expr.rég, Grade, ignore.case = TRUE, perl = TRUE)] 
  ...
  if (expr.rég != "") {
    ...
    A.non.cat <- Lignes_A[! grepl(expr.rég, Grade, ignore.case = TRUE, perl = TRUE)] 
    if ((N.A.non.cat <<- uniqueN(A.non.cat$Matricule)) > 0) {
