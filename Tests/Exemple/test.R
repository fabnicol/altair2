...
sauvebase <- function(x, y) {
  Sauv.base(file.path(chemin.dossier.bases, prime$dossier),
  x,
  y,
  FALSE,
  sep = ";",
...
essayer({
  sauvebase("personnels.A.B", "personnels." %+% tolower(ident_prime) %+% "." %+% tolower(prime$prime_B))
  sauvebase("A.non.cat", ident_prime %+% ".non.cat" %+% prime$catégorie)
  sauvebase("A.non.tit", ident_prime %+% ".non.tit")
