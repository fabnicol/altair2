...
e.adjudant   <- "^.?adju"
e.major      <- "^.?maj"
e.lieut      <- "^lieutenant\\s?(?:[^c-]|c(?!o))*$"
e.cap        <- "^capi"
e.commandant <- "^comm"
e.lieutcol   <- "^l(?:i|t).*col.*$"
e.col        <- "^col"

e.offsup     <- e.commandant %+% "|" %+% e.lieutcol %+% "|" %+% e.col
e.offsub     <- e.cap %+% "|" %+% e.lieut %+% "|" %+% e.major
e.sousoff    <- e.sergent %+% "|" %+% e.adjudant
e.gradsap    <- e.sapeur %+% "|" %+% e.caporal

e.santé <- "(?:m[eé]d|pharm|v[eé]t[eé]ri).*"

# Les expert biomédicaux ou psychologues sont par simplification classés comme capitaines

e.santé.cap        <- "(?:infirm|expert)" %+% "|" %+% e.santé %+% "(?:capi|2.*cla)"
e.santé.commandant <- e.santé %+% "(?:comm|1.*cla)"
e.santé.lieutcol   <- e.santé %+% "(?:hors.*cla|l(?:i|t).*col.*$)"
e.santé.col        <- e.santé %+% "(?:cla.*ex|col)"

e.santé.offsup  <- e.offsup  %+% "|" %+% e.santé.col %+% "|" %+% e.santé.lieutcol %+% "|" %+% e.santé.commandant
e.santé.offsub  <- e.offsub  %+% "|" %+% e.santé.cap

services <- unique(Bulletins.paie$Service)
sssm <- services[grepl("SSSM", services)]
autres.services <- setdiff(services, sssm)


EQTP.tot     <- eqtp.grade(période = période)
EQTP.SPP <- eqtp.grade(période = période,
                      classe = c(e.sapeur,
                      e.caporal,
                      e.sergent,
                      e.adjudant,
                      ...
                      "MAJOR",
                      "LIEUTENANT",
                      "MEDECIN/PHARM./VET./INFIRM. équ. CAP.",
                      "CAPITAINE",
                      "MEDECIN/PHARM./VET. équ. COMDT.",
                      "COMMANDANT",
                      "MEDECIN/PHARM./VET. équ. LT-COL.",
                      "LIEUTENANT-COLONEL ou équ. méd.",
                      "MEDECIN/PHARM./VET. équ. COL.",
                      "COLONEL ou équ. méd."),
                      agr = FALSE)

EQTP.SPP.agr <- eqtp.grade(période = période,
                            classe = c(e.sapeur,
                            e.caporal,
                            e.sergent,
                            e.adjudant,
...
Tab5.EQTP.SPP.hors.SSM <- eqtp.grade(période = période,
                                      classe = c(e.offsup,
                                      e.offsub,
                                      e.sousoff,
                                      e.gradsap),
...
Tab4.COUT.SPP.quotnulle <- charges.eqtp(période = période,
                                             classe = c(e.offsup,
                                                        e.offsub,
                                                        e.sousoff,
                                                        e.gradsap),
...
Tab4.COUT.PATS.A <- charges.eqtp(période = période,
                               grade = grades.pats,
                               catégorie = "A",
                               exclure.codes =  c("0999", "2950"))

Tab4.COUT.PATS.B <- charges.eqtp(période = période,
                                 grade = grades.pats,
                                 catégorie = "B",
                                 exclure.codes =  c("0999", "2950"))

Tab4.COUT.PATS.C <- charges.eqtp(période = période,
                                 grade = grades.pats,
                                 catégorie = "C",
                                 exclure.codes =  c("0999", "2950"))

Tab4.COUT.PATS <- charges.eqtp(période = période,
                                 grade = grades.pats,
                                 exclure.codes =  c("0999", "2950"))

Tab4.COUT.PATS.ABC <- rbind(Tab4.COUT.PATS.A[Grade == "Moyenne"], Tab4.COUT.PATS.B[Grade == "Moyenne"], Tab4.COUT.PATS.C[Grade == "Moyenne"])
Tab4.COUT.PATS.ABC <- rbind(Tab4.COUT.PATS.ABC, Tab4.COUT.PATS[Grade == "Moyenne"])
Tab4.COUT.PATS.ABC[ , 1] <- c("A", "B", "C", "Moyenne")
...
#'   
#'### Tableau n°4 : L’évolution 2011-2016 du coût salarial, charges comprises, par EQTP (SPP hors SSM)     
#'   

kable(Tab4.COUT.SPP.hors.SSM)

#'   
#'### Tableau n°4 bis : L’évolution 2011-2016 du coût salarial, charges comprises, par EQTP (SSM)     
#'   
...
#'   
#'### Tableau n°4 ter : L’évolution 2011-2016 du coût salarial total, charges comprises (SPP), quotités nulles     
#'   

kable(Tab4.COUT.SPP.quotnulle)

#'    
#'[Lien vers le tableau des coûts salariaux total quotités nulles](Bases/SDIS/Tab4.COUT.SPP.quotnulle.csv)      
#'    

#'   
#'### Tableau n°4 quater : L’évolution 2011-2016 du coût salarial, charges comprises, par EQTP (PATS) et par catégorie statutaire     
#'   

kable(Tab4.COUT.PATS.ABC)

#'    
#'[Lien vers le tableau des coûts salariaux PATS](Bases/SDIS/Tab4.COUT.PATS.ABC.csv)      
#'    
...
kable(Tab5.EQTP.SPP.hors.SSM)


#'   
#'### Tableau n°5 bis : L’évolution 2011-2016 des effectifs de sapeurs-pompiers (SSM)   
#'   
...
#'    
#'[Lien vers le tableau des EQTP tous grades](Bases/SDIS/EQTP.tot.csv)       
#'[Lien vers le tableau des EQTP SPP, tous grades](Bases/SDIS/EQTP.SPP.agr.csv)         
#'[Lien vers le tableau des EQTP SPP, par grade](Bases/SDIS/EQTP.SPP.csv)      
#'[Lien vers le tableau n°5 SPP hors SSM](Bases/SDIS/Tab5.EQTP.SPP.hors.SSM.csv)      
#'[Lien vers le tableau n°5 SPP SSM](Bases/SDIS/Tab5.EQTP.SPP.SSM.csv)       
#'[Lien vers le tableau n°5 contractuels SSM](Bases/SDIS/Tab5.EQTP.SSM.contr.csv)        
...
# Entrées.SPP  <- flux("SPP", io = "in")
# Entrées.PATS <- flux("PATS", io = "in")
# Entrées.total <- flux("Total", io = "in")


#'   
#'### Tableau n°7 : Entrées-sorties SPP
#'   

kable(Sorties.SPP)

#'    
#'[Lien vers le tableau des sorties SPP](Bases/SDIS/Sorties.SPP.csv)    
#'[Lien vers le tableau des sorties PATS](Bases/SDIS/Sorties.PATS.csv)     
#'[Lien vers le tableau des sorties SPP/PATS](Bases/SDIS/Sorties.total)                                  
#'   
...
ES <- CRC_ES[Entrée < 2007 
             & Sortie > 2016 
             & Année == 2016 
             & Mois == 12][ , Matricule := as.character(Matricule)]
...
série <- function(cat) {
  ...
  filtre <- if (cat == "Professionnel") {
    function(Grade, Catégorie, Type.personnel) grepl(pattern = e.santé.offsup,
                                                     Grade,
                                                     perl = TRUE,
...
ES_SPP_PATS <- rbind(c("Officiers SPP", série(cat = "Professionnel")),
                     c("PATS cat. A", série(cat = "PATS")))
#'   
#'Tableau n°8 : mobilité des SPP et PATS    
#'    

kable(data.table(ES_SPP_PATS), 
      align = "r",
