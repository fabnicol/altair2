...
if (table.rapide) {
  ...
  Paie <- data.table::data.table(Paie)
  Bulletins.paie.nir.total.hors.élus <- unique(Paie[Paie$Année == fin.période.sous.revue
                                             & Paie$Mois == 12
                                             & Paie$Statut != "ELU",
...
if (length(temp > 0))
  hist(temp,
       xlab = "Rémunération indemnitaire imposable en milliers d'euros \n",
       ylab = "Effectif",
       xlim = c(0, 40),
