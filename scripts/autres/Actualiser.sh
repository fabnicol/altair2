...
for i in lib sft data Docs Interface fwidgets_lib lhx Interface_linux linux '*.txt' '*.R' '*.sh' '*.desktop' VERSION LICENCE '*.ico' '*.bmp' '*.png'  postinstall.sh altaïr.Rproj 'scripts/R' 'scripts/R/Docs' sys
do
rm -rf "$i"
...
sudo chmod 0770 /home/jf/Dev/altair/scripts/R/*.*
...
do 
   if test -d /home/$i; then
     ...
     sudo chown -vR $i "/home/$i/Dev/altair/scripts/R/Donnees/xhl/$i"
