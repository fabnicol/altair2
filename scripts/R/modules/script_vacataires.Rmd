...
```{r Trouver, pour toutes les lignes de paie non-charges, une indication de proportionnalité horaire explicite}
...
```
```{r Produire la liste de ces libellés horaires}
...
```
```{r Vérifier si des fonctionnaires titulaires ou stagiaires bénéficient de vacations horaires et donner les caractéristiques}
...
```{r tests-statutaires-vacations-ri}
```
