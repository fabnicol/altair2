...
{
  if (dossier == "" || nom == "" || is.null(dossier) || is.null (nom)) {
    message("Problème avec: " %+% dossier %+% " ou: " %+% nom)
    return(FALSE)
  }
...
#'
#' @return Valeur incrémente de numero.tableau
#' @export
...
#' Fusionner deux environnements.
#' @param e  Environnement 1
#' @param envir   Environnement 2
#' @return Environnement fusionné.
#' @examples c <- merge_environments(a, b)
#' @export
#'
merge_environments <- function(e, envir) {
  as.environment(c(as.list(e), as.list(envir)))
}


#' Fusionner un environnement à l'environnement global.
#' @param e  Environnement
#' @return Liste (non nommée) des objets ajoutés à .GlobalEnv
#' @examples c <- merge_environment(e)
#' @export
#'
merge_environment <- function(e) {
  if (! is.environment(e)) {
    if (is.list(e)) {
      e <- e[[1]]
    } else {
  ...
  if (!is.null(e) && is.environment(e)) {
      lapply(ls(e), function(u) assign(u, e[[u]], envir = globalenv())) 
  }
}
...
#' @param fonction Nom de fonction
...
#' @param envir Environment de la function oi sinon NULL
...
if (! is.null(chemin) && get(gsub(".R", "", basename(chemin), fixed = TRUE), envir = globalenv()) == FALSE) invisible(return(NULL))

invisible(sapply(index, function(x) {

  assign(variable, x, envir = globalenv())
  ...
  if (is.null(fonction)) {
    ...
    if (gen) {
      ...
      if (is.null(envir)) L <- list() else L <- list(envir)
      for (f in fonction[[1]]) {
        do.call(get(f), L)
      }
      return(envir)
