...
smpt <- function(Cat = NULL, Liste_statuts = NULL, type =  "smpt net") {
  ...
  if (!is.null(T) && T[1] != "") list(T, S_) else NULL
}
...
calcul_rmpp <- function(e) {

    temp <- e$Analyse.variations[nb.jours > seuil.troncature
                               & ! is.na(Montant.net.annuel.eqtp)
                               & Montant.net.annuel.eqtp  > minimum.positif
                               & ! is.na(Statut)]
    
    assign("Analyse.variations", temp, envir = e)

    q3 <- quantile(temp$variation.rémunération,
...
assign("Anavar.synthese", e$Analyse.variations[total.jours > seuil.troncature
                                      & pris.en.compte == TRUE
                                      & ! is.na(statut.fin.annee)
                                      & ! is.na(variation.rémunération)
                                      & variation.rémunération > q3[[1]]
                                      & variation.rémunération < q3[[2]]], envir = e)

assign("Anavar.synthese.plus.2.ans",
       e$Anavar.synthese[! is.na(plus.2.ans) & plus.2.ans == TRUE], envir = e)

assign("Anavar.synthese.moins.2.ans", 
       e$Anavar.synthese[! is.na(plus.2.ans) & plus.2.ans == FALSE], envir = e)

assign("masque.présent.sur.periode", 
       bitwShiftL(1, duree.sous.revue - 1), envir = e)    

sauv.bases("Remunerations", e, "Anavar.synthese")
}
