...
# Modélisation : en première année sous revue, est réputé absent le Matricule ayant une quotite non uniformément nulle sur l'annee, qui n'a pas de quotité positive dans les trois premiers mois
# Modélisation : en dernière année sous revue, est réputé absent le Matricule ayant une quotite non uniformément nulle sur l'annee, qui n'a pas de quotité positive dans les trois derniers mois
...
#' En première année sous revue, est réputé absent le Matricule ayant une quotité non uniformément nulle sur l'annee, qui n'a pas de quotite positive dans les trois premiers mois
#' En dernière année sous revue, est réputé absent le Matricule ayant une quotité non uniformément nulle sur l'annee, qui n'a pas de quotite positive dans les trois derniers mois
...
#'                 \item{\code{ETPT entrants}}{ETPT des entrants de l'année, par annee.}
#'                 \item{\code{ETPT sortants}}{ETPT des sortants de l'année, par annee.}
...
#'                 \item{\code{Var. effectifs}}{différence du nombre d'entrants et de sortants, par ann"e, puis Total.}
...
#'               \describe{
#'                 \item{\code{Annee}}{Annees de la periode sous revue, puis le libellé "Total".}
#'                 \item{\code{Effet noria}}{Cumul des économies (si négatif) ou des coûts (si positif) sur salaires (nets ou bruts) dus au remplacement des sortants par les entrants, par ann"e, puis Total.}
...
#'                 Est positif si les entrants intègrent l'organisme en moyenne plus tôt dans l'annee que les sortants ne le quittent, par année, puis Total.}                                  
...
#' 
#'               \describe{
#'                 \item{\code{Annee}}{Années de la période sous revue}
#'                 \item{\code{RMPP}}{Rémunération moyenne des personnes en place, présentes deux ann"es complètes consécutives.\cr
#'                  La vérification de la permanence de la quotit" sur les deux ann"es n'est pas implémentée.}
#'                 \item{\code{Entrée n - 1}}{Effet relatif en \% des entr"es de personnels en n - 1 présents en n et des variations de quotite entre n - 1 et n. Pour la première ann"e, n - 1 est remplacé par n + 1 et "entr"es" par "sorties".}
...
#'               \describe{
#'                 \item{\code{periode}}{Années de la p"riode sous revue, puis le libellé "Total".}
...
#'                 \item{\code{variation.effectifs}}{Difference du nombre d'entrants et de sortants, par année, puis Total.}
...
#'                 \item{\code{effet.vacances}}{Cumul des économies (si négatif) ou des coûts (si positif) sur salaires (nets ou bruts) dus aux vacances d'emplois.\cr
#'                  Est positif si les entrants intègrent l'organisme en moyenne plus tôt dans l'annee que les sortants ne le quittent, par année, puis Total.}                                                   
#'                 \item{\code{effet.total.entrees.sorties}}{Cumul des effets précédents, par année, puis Total.}
...
#'                 \item{\code{coef.entrees.ant}}{Effet relatif en \% des entrees de personnels au cours de l'année précédente.}
...
if (! is.null(filtre)) {
  if (filtre[1] == "A" | filtre[1] == "B" | filtre[1] == "C") {
    Base <- Base[Categorie == filtre[1]]
  } else {
...
filtrage <- function(annee) {
    ...
    if (! is.null(filtre)) {
      ...
      if (filtre[1] == "A" | filtre[1] == "B" | filtre[1] == "C") {
	          présents.bulletins[[transl(annee)]] <<- Bulletins[Categorie == filtre[1] 
                                                          & Annee == annee 
                                                          & quotite.moyenne != 0 
                                                          & Grade != "A" 
