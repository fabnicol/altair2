...
tex2pdf <- function(infile = "altair.tex", 
  ...
  if (ouvrir.document && file.exists(cle_outfile)) {
    ...
    if (setOSWindows) {
	  cat("Opening Adobe Acrobat...")
	        tryCatch({
			           processx::run("Acrobat.exe",  args=cle_outfile, error_on_status=FALSE)
			         },
					 error = function(cond){
						...
						tryCatch({processx::run(file.path("C:\\Program Files","Adobe","Acrobat DC","Acrobat","Acrobat.exe"), 
												args=cle_outfile,
