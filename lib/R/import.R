...
quotites <- function() {
  ...
  Bulletins.paie <- merge(Bulletins.paie,
                          Paie[ , .(Filtre_actif = Filtre_actif[1]),  by = c("Nir", "Annee", "Mois")],
                          by = c("Nir", "Annee", "Mois"),
                          all.x = TRUE,
                          all.y = FALSE)
  
  message("Quotités calculees a")
  ...
  message("Quotités calculees c") 
  
  # L'écrêtement des quotites est une contrainte statistiquement discutable qui permet
  # de "stresser" le modèle
  # Par défaut les quotites sont écrêtées pour pouvoir par la suite raisonner 
  # en définissant le temps plein comme quotite == 1

  if (écreter.quotites) {

    Bulletins.paie[ , quotite   :=  ifelse(MHeures < minimum.positif, NA,
                                           ifelse(Heures > MHeures, 1, round(Heures/MHeures, digits = 2)))]

  } else {

    Bulletins.paie[ , quotite   :=  ifelse(MHeures < minimum.positif, NA,
                                           round(Heures/MHeures, digits = 2))]
  }

  Bulletins.paie[Statut == "ELU", `:=`(MHeures = 1,
                                       quotite = 1)]

  message("Quotités calculees ... OK")
...
#' Normaliser les NIR
#' @note Cette fonction récupère les NIR existant plus tarden cas de valeur manquante un année 
#' et en cas d'incohérences sélectionne le dernier NIR en date.
#' @export

normaliser.nir <- function() {

  # Les bases sont déjà triées par Année, Mois
  # donc .N va prendre le dernier Nir dans le temps pour toute maire (Matricule, Nom)
  
  Paie[ , Nir := Nir[.N], keyby = c("Matricule", "Nom")]
  
  Bulletins.paie[ , Nir := Nir[.N], keyby = c("Matricule", "Nom")]
...
importer_ <- function(normaliser.colonnes = TRUE) {
  ...
  if (normaliser.colonnes) {
    ...
    setkey(Paie, Matricule, Nom, Annee, Mois)
    setkey(Bulletins.paie, Nom, Matricule, Annee, Mois)
  }  else  {
  
    setkey(Paie, Matricule, Nom, Année, Mois)
    setkey(Bulletins.paie, Nom, Matricule, Année, Mois)
    return("")
  }
  ...
  # Certains NIR sont parfois problématiques (inexistants ou faux) , ce qui peut poser des problèmes divers.
