...
\protect\caption{The default PDF device.\label{mar:pdf-dev}}
\end{marginfigure}
...
\protect\caption{The PNG device.\label{mar:png-dev}}
\end{marginfigure}
...
\protect\caption{Two high-level plots are captured. The key to arrange two plots side
...
\protect\caption{Two complete R expressions will produce at most two plots, as long
...
\protect\caption{Low-level plot changes in base graphics can be recorded separately,
...
\protect\caption{Two plots were produced in this chunk, but only the last one is kept.
...
\protect\caption{A clock animation. You have to view it in Adobe Reader: click to play/pause;
...
\protect\caption{A left-aligned plot adapted from \texttt{?stars} (I call this the
...
\protect\caption{A plot created by \textbf{tikzDevice} with math expressions and typewriter
...
\protect\caption{The original plot produced in R, with a large margin.\label{fig:pdf-nocrop}}
\end{figure}
...
\protect\caption{The cropped plot; it fits better in the document.\label{fig:pdf-crop}}
\end{figure}
