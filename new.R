...
Vérifier_non_annexe2 <- function(Montant, Année) if (Année == 2013)  (Montant > 3361) else if (Année == 2012) (Montant > 3322) else if(Année == 2011) (Montant > 3222) else if (Année == 2010) (Montant > 3169) else if (Année == 2009) (Montant > 3132)  else (Montant > 3076)

Analyse.rémunérations2 <- Paie[ , .(Nir          = Nir[1],
...
Analyse.rémunérations2[ ,  Filtre_non_annexe := Vérifier_non_annexe2(Montant.net.annuel, Année) | ( nb.mois > 1 
                      & cumHeures > 120 
                      & cumHeures / nb.jours > 1.5)]

Analyse.rémunérations2[ , `:=`(rémunération.indemnitaire.imposable = indemnités + sft + indemnité.résidence + rémunérations.diverses,
...
Analyse.rémunérations2[ , indemnités.élu := ifelse(Statut == "ELU", total.lignes.paie, 0)]
