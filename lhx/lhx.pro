...
#       FGETC_PARSING                       # parcourir les fichiers par ifstream (C++)
        STRINGSTREAM_PARSING                # mise en mémoire vive des fichiers de paye par std::ostringstream (consomme plus de mémoire vive et accélère le 1er traitement)
#       MMAP_PARSING                        # parcourir les fichiers par mappage mémoire (C, unix uniquement).
